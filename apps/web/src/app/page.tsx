      ...
      <main className="relative flex min-h-screen flex-col items-center justify-center overflow-hidden bg-[rgb(var(--color-sidebar-bg))]">
        ...
        <div className="relative z-10 flex flex-col items-center gap-8">
          ...
          <div className="flex items-center gap-4 pt-4">
            ...
            <a
              href="mailto:access@sovereignos.dev?subject=Request%20Access%20to%20SOVEREIGN"
              className="inline-flex items-center justify-center gap-2 rounded-lg border border-white/20 bg-white/5 px-8 py-3 text-sm font-semibold text-white transition-all duration-200 hover:bg-white/10 hover:border-white/30"
            >
              Request Access
            </a>
          </div>

          {/* Feature pills — linked to docs */}
          <div className="flex flex-wrap items-center justify-center gap-2 pt-8">
            {[
              { name: "Agent Studio", href: "/docs/agents" },
              { name: "Workflow Orchestration", href: "/docs/runs" },
              { name: "Policy Engine", href: "/docs/policies" },
              { name: "Audit Trail", href: "/docs/policies" },
              { name: "Multi-Tenant", href: "/docs/getting-started" },
              { name: "MCP Connectors", href: "/docs/connectors" },
              { name: "Mobile Terminal", href: "/terminal" },
              { name: "Multi-Provider AI", href: "/docs/agents" },
            ].map((feature) => (
              <a
                key={feature.name}
                href={feature.href}
                className="rounded-full border border-white/10 bg-white/5 px-3 py-1 text-xs text-[rgb(var(--color-sidebar-text))] transition-colors hover:bg-white/10 hover:border-white/20"
              >
                {feature.name}
              </a>
            ))}
